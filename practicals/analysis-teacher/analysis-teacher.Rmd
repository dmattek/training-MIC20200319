...
```{r, echo = T, eval = T}
dtEGFwithExp = merge(dtEGF,
                     dtEGFexp,
                     by.x = "Metadata_Series",
                     by.y = "Position")
...
```{r,echo=T, eval = T}
vInFiles = list.files(path = "../data/original", 
                      pattern = "csv.gz",
                      full.names = T)
vInFiles
```
...
```{r,echo=T, eval = T}
dtAll[,
      Stim_Treat := gsub(".*_(.*)sust_.*", "\\1", fileName)]
...
```{r, echo = T, eval = T}
dtAll[, 
      (lCol$trackiduni) := sprintf("%s_%02d_%04d",
                                   get(lCol$treatGF),
                                   get(lCol$fov),
                                   get(lCol$trackid))]
...
require(imputeTS)
dtAll[, 
      (lCol$meas) := imputeTS::na.interpolation(get(lCol$meas)), 
      by = c(lCol$trackiduni)]
```
...
```{r, echo = T, message = F, eval = T, fig.align='center', fig.width= 10, fig.height=8}
require(plotly)
...
```{r, echo = T, eval = F}
dt[frame %in% c(10, 25, 30)]
```

Use and combine `ggplot2` functions such as `geom_boxplot`, `geom_violin`, `geom_dotplot`, e.g.:

```{r, echo = T, eval = T, fig.align='center', fig.width= 10, fig.height=8}
...
dtAllWide = data.table::dcast(dtAll, 
                              get(lCol$trackiduni) ~ get(lCol$frame), # place for the formula; LHS - row variable, RHS - column variable
                              value.var = lCol$measNorm) # place for the column name that will be cast into columns

# obtain row names for later
vsRowNames = dtAllWide[["lCol"]]
...
# check the result (susbection only)
head(mAllWide[1:10, 1:10])
```
...
```{r, echo = T, eval = T, message = F, fig.align='center', fig.width= 8, fig.height=8}
heatmap(mAllWide)
```
...
```{r, echo = T, eval = T}
dtClAssign = as.data.table(clAssign, 
                           keep.rownames = T)
setnames(dtClAssign, 
         c(lCol$trackiduni, 
           lCol$clid))
...
```{r, echo = T, eval = T, fig.align='center', fig.width= 10, fig.height=5}
ggplot(dtAllCl, 
       aes_string(x = lCol$frame, 
                  y = lCol$measNorm, 
                  group = lCol$trackiduni)) +
  ...
  stat_summary(
    ...
    fun = mean, # place for a function to calculate the summary
    geom = "line", group = 1,
    colour = 'red', linetype = 'solid', size = 1) +
  facet_grid(get(lCol$treatGF) ~ get(lCol$treatConc)) + # place for a faceting variable
  theme_minimal() +
...
```{r, echo = T, eval = T}
dtAllClN = dtAllCl[, 
                   .(nTimeSer = length(unique(get(lCol$trackiduni)))),
                   by = c(lCol$treatGF, 
                          lCol$treatConc,
                          lCol$clid)]

# Order by GF, concentration, and cluster number
setorderv(dtAllClN,
          c(lCol$treatGF, 
            lCol$treatConc,
            lCol$clid))
...
```{r, echo = T, eval = T, message = F, fig.align='center', fig.width= 10, fig.height=7}
# for percentages on y-axis in ggplot
require(scales)

# The core plot: concentrations on the x-axis, the number of time series on y-axis
p5 = ggplot(dtAllClN, 
            aes_string(x = lCol$treatConc, # places for x and y variables
                       y = "nTimeSer"))

# Facetting per growth factor
p5 = p5 +
  facet_wrap(~ get(lCol$treatGF))
...
p5 = p5 +
  geom_bar(aes_string(fill = lCol$clid), # place for variable to colour the bars
           stat = "identity", 
...
dtAllWide = dcast(dtAll, 
                  get(lCol$trackiduni) ~ get(lCol$frame), 
